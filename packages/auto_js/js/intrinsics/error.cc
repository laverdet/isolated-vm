...
class specific_error_value : public error {
	public:
		template <class... Args>
		explicit specific_error_value(util::format_string<Args...> format, Args&&... args) :
				error{Type},
				message_{util::transcode_string<char16_t>(std::string_view{util::format(format, std::forward<Args>(args)...)})} {}
