...
class task_runner : public v8::TaskRunner {
	public:
		auto post_idle(idle_task task) -> void;
		auto post_non_nestable_delayed(task_type task, steady_clock::duration timeout) -> void;
		auto post_non_nestable(task_type task) -> void;
		virtual auto post_delayed(task_type task, steady_clock::duration timeout) -> void = 0;
...
// NOLINTNEXTLINE(readability-convert-member-functions-to-static)
auto task_runner::post_non_nestable_delayed(task_type /*task*/, steady_clock::duration /*timeout*/) -> void {
	std::unreachable();
};
...
// task_runner_of<T>
template <class Self>
auto task_runner_of<Self>::IdleTasksEnabled() -> bool {
	return &Self::post_idle != &task_runner::post_idle;
}

template <class Self>
auto task_runner_of<Self>::NonNestableTasksEnabled() const -> bool {
	return &Self::post_non_nestable != &task_runner::post_non_nestable;
}

template <class Self>
auto task_runner_of<Self>::NonNestableDelayedTasksEnabled() const -> bool {
	return &Self::post_non_nestable_delayed != &task_runner::post_non_nestable_delayed;
}

template <class Self>
auto task_runner_of<Self>::PostTaskImpl(task_type task, const v8::SourceLocation& /*location*/) -> void {
	return self().post(std::move(task));
}

template <class Self>
auto task_runner_of<Self>::PostNonNestableTaskImpl(task_type task, const v8::SourceLocation& /*location*/) -> void {
	return self().post_non_nestable(std::move(task));
}

template <class Self>
auto task_runner_of<Self>::PostDelayedTaskImpl(task_type task, double delay_in_seconds, const v8::SourceLocation& /*location*/) -> void {
	auto duration = duration_cast<steady_clock::duration>(std::chrono::duration<double>{delay_in_seconds});
	return self().post_delayed(std::move(task), duration);
}

template <class Self>
auto task_runner_of<Self>::PostNonNestableDelayedTaskImpl(task_type task, double delay_in_seconds, const v8::SourceLocation& /*location*/) -> void {
	auto duration = duration_cast<steady_clock::duration>(std::chrono::duration<double>{delay_in_seconds});
	return self().post_non_nestable_delayed(std::move(task), duration);
}

template <class Self>
auto task_runner_of<Self>::PostIdleTaskImpl(idle_task task, const v8::SourceLocation& /*location*/) -> void {
