...
auto clock_time(const isolated_v8::realm_scope& /*realm*/) -> double {
	auto now = std::chrono::utc_clock::now();
	return duration_cast<js_clock::duration>(now.time_since_epoch()).count();
}

auto performance_time(const isolated_v8::realm_scope& /*realm*/) -> double {
	auto now = std::chrono::steady_clock::now();
	return duration_cast<js_clock::duration>(now.time_since_epoch()).count();
}

runtime_interface::runtime_interface(const isolated_v8::agent_lock& agent) :
		clock_time_{make_unique_remote(agent, js::iv8::function_template::make(agent, js::free_function{clock_time}))},
		performance_time_{make_unique_remote(agent, js::iv8::function_template::make(agent, js::free_function{performance_time}))} {
}

auto runtime_interface::instantiate(const isolated_v8::realm_scope& realm) -> v8::Local<v8::Module> {
