...
export class script_handle {
	public:
		...
		auto run(environment& env, realm_handle& realm, run_script_options options) -> js::forward<js::napi::value<>>;

		static auto class_template(environment& env) -> js::napi::value<class_tag_of<script_handle>>;
		static auto compile_script(agent_handle& agent, environment& env, js::string_t code_string, compile_script_options options) -> js::forward<js::napi::value<>>;
