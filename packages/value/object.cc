...
struct accept<Meta, Type> {
		...
		constexpr auto operator()(dictionary_tag /*tag*/, auto&& dictionary) const -> Type {
			using value_type = std::ranges::range_value_t<decltype(dictionary)>::second_type;
			hash_type checksum{};
			Type subject;
			...
			for (auto&& [ key, value ] : dictionary) {
				...
				if (descriptor != nullptr) {
					...
					(descriptor->second)(*this, subject, value);
				}
			}
		...
		consteval static auto make_property_map() {
			...
			auto initial_property_map = prehashed_string_map<initial_mapped_type, properties.size()>{
				...
				std::views::transform([](const auto& entry) {
					auto is_optional = std::get<0>(entry);
					auto key = std::get<1>(entry);
					auto acceptor = std::get<2>(entry);
					return std::pair{key, std::pair{is_optional, acceptor}};
				})
			};
