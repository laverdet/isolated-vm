...
auto object::into_range() const -> range_type {
	if (keys_ == array{}) {
		keys_ = array{env(), napi_invoke_checked(napi_get_property_names, env(), **this)};
	}
	return keys_ | std::views::transform(iterator_transform{*this});
}

auto object::get(napi_value key) const -> napi_value {
	return napi_invoke_checked(napi_get_property, env(), *this, key);
}

auto object::has(napi_value key) const -> bool {
	return napi_invoke_checked(napi_has_own_property, env(), *this, key);
}
